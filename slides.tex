...
\begin{frame}[label={sec:orge4d135e}]{NOT to talk about}
\begin{columns}
...
\section{The problem}
\label{sec:orgc86c144}
\begin{frame}[label={sec:orgb90add0}]{Pharmacokinetics (PK) data}
\begin{figure}[htbp]
\centering
...
\begin{frame}[label={sec:org1627c86}]{A Pharmacokinetics (PK) model}
\begin{align*}
...
\begin{frame}[label={sec:org4aafb38}]{A statistical model}
\begin{align*}
...
\begin{frame}[label={sec:orgdaf8db1}]{Estimate \(\theta\)}
\begin{itemize}
...
\begin{frame}[label={sec:org3f8c88d}]{Prior \& likelihood}
\begin{align*}
  \log{p(\theta|y)} &= C + \log{p(y | \theta)} + \log{p(\theta)}
\end{align*}
\begin{center}
...
\begin{frame}[label={sec:org28c5e80}]{Sample the posterior}
...
\begin{frame}[label={sec:orgb00c110}]{Posterior samples: trace}
\begin{center}
\includegraphics[width=\textwidth]{./figure/twoCpt_trace.pdf}
\end{center}
\end{frame}

\begin{frame}[label={sec:org192f2eb}]{Posterior samples: density}
\begin{center}
...
\begin{frame}[label={sec:orgb156654}]{The Metropolis-Hastings algorithm}
...
\begin{frame}[label={sec:orgb583b8a}]{Why MCMC?}
\begin{quote}
...
\label{sec:orgcd71e35}
\begin{frame}[label={sec:orgdc16274}]{The problem}
\begin{center}
\includegraphics[width=\textwidth]{./figure/twocpt_pop_ppc1.pdf}
\end{center}
\end{frame}

\begin{frame}[label={sec:org105090f}]{Hierarchical model}
\begin{columns}
...
\begin{frame}[label={sec:org03f0998}]{Challenges: high dimensional gaussian distribution}
...
\begin{frame}[label={sec:org91fd278}]{Challenges: Geometry of posterior}
\begin{columns}
\begin{column}{0.4\columnwidth}
\begin{align*}
  \theta_0 &= 0,\\
...
\label{sec:orgd6840a5}
\begin{frame}[label={sec:orgc4045a2}]{Hamiltonian Monte Carlo}
\begin{align*}
...
\begin{frame}[label={sec:org9e0bfd5}]{Hamiltonian Monte Carlo}
\begin{align*}
  H(\theta, r) &= -\log{p(r, \theta | y)} = T(r) + V(\theta) = -\log{r} - \log{p(\theta|y)},\\
  \frac{d\theta}{dt} &= \frac{\partial H}{\partial r},\qquad
  \frac{dr}{dt} = -\frac{\partial H}{\partial \theta},
...
\begin{frame}[label={sec:org239f5c9}]{A principled sampler for \(p(\theta|y)\)}
\begin{align*}
...
\label{sec:orga38e84d}
\begin{frame}[label={sec:orgb75b40a}]{A tale of two ODEs}
...
\begin{frame}[label={sec:org5094dec}]{Automatic differentiation (1 obsv/subject: k=1)}
\begin{center}
\includegraphics[width=\textwidth]{./figure/autodiff_0_diag.pdf}
\end{center}
\end{frame}

\begin{frame}[label={sec:org32da1b6}]{Automatic differentiation (1 obsv/subject: k=1)}
\begin{center}
...
\begin{frame}[label={sec:orgcfaf1c3}]{Automatic differentiation}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{center}
...
\begin{frame}[label={sec:org98cb013}]{Sensitivity solution}
\begin{align*}
...
\begin{frame}[label={sec:org264eb25}]{Thanks}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{center}
