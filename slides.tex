...
\begin{frame}[label={sec:orgadf54cb}]{NOT to talk about}
\begin{columns}
...
\section{The problem}
\label{sec:org590b80a}
\begin{frame}[label={sec:orgd60fbb9}]{Pharmacokinetics (PK) data}
\begin{figure}[htbp]
\centering
...
\begin{frame}[label={sec:org6777ec9}]{Posterior samples: trace}
\begin{center}
\includegraphics[width=\textwidth]{./figure/twoCpt_trace.pdf}
\end{center}
\end{frame}

\begin{frame}[label={sec:org6cf283c}]{Posterior samples: density}
\begin{center}
...
\begin{frame}[label={sec:org95d0d87}]{The Metropolis-Hastings algorithm}
...
\begin{frame}[label={sec:org6b66099}]{Why MCMC?}
\begin{quote}
...
\label{sec:orgbf31f9c}
\begin{frame}[label={sec:org1b1e932}]{The problem}
\begin{center}
\includegraphics[width=\textwidth]{./figure/twocpt_pop_ppc1.pdf}
\end{center}
\end{frame}

\begin{frame}[label={sec:org7ea550d}]{Hierarchical model}
\begin{columns}
...
\begin{frame}[label={sec:org9ffc2da}]{Challenges: high dimensional gaussian distribution}
...
\begin{frame}[label={sec:org76207a7}]{Challenges: Geometry of posterior}
\begin{columns}
\begin{column}{0.4\columnwidth}
\begin{align*}
  \theta_0 &= 0,\\
...
\label{sec:org1f3ae6f}
\begin{frame}[label={sec:org8eea16e}]{Hamiltonian Monte Carlo}
\begin{align*}
...
\begin{frame}[label={sec:org81d1e91}]{Hamiltonian Monte Carlo}
\begin{align*}
  H(\theta, r) &= -\log{p(r, \theta | y)} = T(r) + V(\theta) = -\log{r} - \log{p(\theta|y)},\\
  \frac{d\theta}{dt} &= \frac{\partial H}{\partial r},\qquad
  \frac{dr}{dt} = -\frac{\partial H}{\partial \theta},
...
\begin{frame}[label={sec:org89ebb9c}]{Automatic differentiation}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{center}
...
\begin{frame}[label={sec:orga723fe8}]{Sensitivity solution}
\begin{align*}
