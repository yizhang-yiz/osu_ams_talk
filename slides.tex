...
\section{The problem}
\label{sec:org9b0ce37}
\begin{frame}[label={sec:org9171c2f}]{Pharmacokinetics (PK) data}
\begin{columns}
...
\begin{frame}[label={sec:orgb840ea1}]{A Pharmacokinetics (PK) model}
\begin{align*}
...
\begin{frame}[label={sec:org0228fcc}]{A statistical model}
\begin{itemize}
...
\begin{frame}[label={sec:org81731df}]{Estimate \(\theta\)}
\begin{itemize}
...
\begin{frame}[label={sec:org4375f46}]{Prior \& likelihood}
\begin{align*}
  \log{p(\theta|y)} &= C + \log{p(y | \theta)} + \log{p(\theta)}
\end{align*}
\begin{center}
...
\begin{frame}[label={sec:orgd2603e9}]{Sample the posterior}
...
\begin{frame}[label={sec:org22e80c2}]{Posterior samples: trace}
\begin{center}
\includegraphics[width=\textwidth]{./figure/twoCpt_trace.pdf}
\end{center}
\end{frame}

\begin{frame}[label={sec:org59391cf}]{Posterior samples: density}
\begin{center}
...
\begin{frame}[label={sec:orge6a87b0}]{The Metropolis-Hastings algorithm}
...
\begin{frame}[label={sec:orgd82c714}]{Why MCMC?}
\begin{quote}
...
\label{sec:orgc0cbd9c}
\begin{frame}[label={sec:org0e9d8b7}]{The problem}
\begin{center}
\includegraphics[width=\textwidth]{./figure/twocpt_pop_ppc1.pdf}
\end{center}
\end{frame}

\begin{frame}[label={sec:org04fe7b6}]{Hierarchical model}
\begin{columns}
...
\begin{frame}[label={sec:org7e13185}]{Challenges: high dimensional gaussian distribution}
...
\begin{frame}[label={sec:org409ff95}]{Challenges: Geometry of posterior}
\begin{columns}
\begin{column}{0.4\columnwidth}
\begin{align*}
  \theta_0 &= 0,\\
...
\label{sec:orga4301fd}
\begin{frame}[label={sec:orgf058999}]{Hamiltonian Monte Carlo}
\begin{align*}
...
\begin{frame}[label={sec:org4c55cd1}]{Hamiltonian Monte Carlo}
\begin{align*}
  H(\theta, r) &= -\log{p(r, \theta | y)} = T(r) + V(\theta) = -\log{r} - \log{p(\theta|y)},\\
  \frac{d\theta}{dt} &= \frac{\partial H}{\partial r},\qquad
  \frac{dr}{dt} = -\frac{\partial H}{\partial \theta},
...
\label{sec:org89b0575}
\begin{frame}[label={sec:org036e498}]{A tale of two ODEs}
...
\begin{frame}[label={sec:org33f2354}]{Automatic differentiation (1 obsv/subject: k=1)}
\begin{center}
\includegraphics[width=\textwidth]{./figure/autodiff_0_diag.pdf}
\end{center}
\end{frame}

\begin{frame}[label={sec:org6631305}]{Automatic differentiation (1 obsv/subject: k=1)}
\begin{center}
